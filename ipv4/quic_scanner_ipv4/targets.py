"""Target input helpers for the IPv4 scanner."""
...
def derive_target_name(input_path: str) -> str:
    """Derive a stable output target name from an input file name."""
    base_name = os.path.splitext(os.path.basename(input_path))[0].lower()
    base_name = re.sub(r"(_|-)?ipv4(_|-)?(test|list)?$", "", base_name)
    base_name = re.sub(r"(_|-)?ips?(_|-)?(test|list)?$", "", base_name)
    base_name = re.sub(r"(_|-)?(test|list)$", "", base_name)
    target = re.sub(r"[^a-z0-9]+", "-", base_name).strip("-")
    return target or "targets"


def resolve_input_path(value: str, input_dir: str = "input") -> str:
    """Resolve either `google_ipv4.txt` or `input/google_ipv4.txt` to a usable path."""
    if not value:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resolve scanner target metadata.")
