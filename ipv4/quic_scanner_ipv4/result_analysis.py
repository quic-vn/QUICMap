#!/usr/bin/env python3
"""Analyze QScanner output and optionally enrich successful IPs with geo data."""
    ...
    with open(stats_file, "w", encoding="utf-8") as handle:
        ...
        if error_rows:
            ...
            for row in error_rows:
                handle.write(f"\t{row.get('address', '')} : {row.get('errorMessage', '')}\n")
...
def pcap_stats(pcap_path: str, out_file: str) -> None:
    ...
    try:
        capture = pyshark.FileCapture(pcap_path, display_filter=hrr_filter)
        hrr_ips = {packet.ip.src for packet in capture if hasattr(packet, "ip")}
        capture.close()
    except Exception:
        hrr_ips = set()
    ...
    with open(out_file, "a", encoding="utf-8") as handle:
        if hrr_ips:
            handle.write("Servers that sent TLS Hello Retry Requests (IPs):\n")
            for ip in sorted(hrr_ips):
                handle.write(f"\t{ip}\n")

        if retry_ips:
            handle.write("Servers that sent a QUIC Retry (IPs):\n")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze ZMap/QScanner results, with optional geo and PCAP enrichment."
    )
...
def main() -> int:
    ...
    qscanner_csv = args.qscanner_csv or os.path.join(out_dir, "qscanner", "output", "quic_connection_info.csv")
