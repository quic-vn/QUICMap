...
def classify_qscanner(df: pd.DataFrame):
    success_mask = df["errorMessage"].isna() | (df["errorMessage"].astype(str).str.len() == 0)
    ...
    with open(stats_file, "w", encoding="utf-8") as f:
        ...
        if not df_err.empty:
            ...
            for _, row in df_err.iterrows():
                f.write(f"\t {row.get('address','')} : {row.get('errorMessage','')}\n")
...
def geo_enrich_ipapi(addresses, out_csv, batch_size=100, sleep_sec=1.0):
    ...
    fields = ["query","status","continent","continentCode","country","countryCode","region",
...
def geo_enrich_ip2location(addresses, out_csv, api_key, sleep_sec=1.0):
    ...
    fields = ["ip","country_code","country_name","region_name","city_name","latitude","longitude",
    ...
    with open(out_csv, "w", encoding="utf-8", newline="") as f:
        ...
        for ip in addresses:
            try:
                ...
                if r.status_code == 200:
                    d = r.json()
                    row = [ip, d.get("country_code",""), d.get("country_name",""), d.get("region_name",""),
...
def pcap_stats(pcap_path: str, out_file: str):
    ...
    HRR_filter = ("tls.handshake.random == "
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="Unified analyzer for ZMap/QScanner results, with optional geo & pcap")
    ap.add_argument("--root", default=".", help="Project root (default: .)")
    ap.add_argument("--out-dir", default="output", help="Output dir (default: output)")
    ap.add_argument("--qscanner-csv", default=None, help="Path to quic_connection_info.csv (default: <out-dir>/qscanner/output/quic_connection_info.csv)")
    ...
    if args.geo != "none":
        succ_ips = list(pd.Series(succ.get("address", [])).dropna().unique())
